...
class CluCHIME:
    ...
    def work_with_sigpyproc_initialiser(self,
                               INT_un,
                               INTnewnorm_whole, 
                               beam):
        self.INT_un=INT_UN
        self.INTnewnorm_wh0le=INTnewnorm_whole
        self.beam=beam
        ...
        INT_un1st =INT_un1st[0,:,:]
        INT_un2nd =INT_un2nd[1,:,:]
        INT_un3rd =INT_un3rd[2,:,:]
        
        INTnewnorm_whole1st =  INTnewnorm_whole1st[0,:,:]
        INTnewnorm_whole2nd =  INTnewnorm_whole2nd[1,:,:]
        INTnewnorm_whole3rd =  INTnewnorm_whole3rd[0,:,:]
        ...
        def create_filterbank_fileobj(tmpfname,
            ...
            fil_fileobj = FileWriter(tmpfname, mode="w", tsamp=tsamp, nchans=nchans, nbits=32)
            fil_fileobj.write(sigproc.encode_header(header))
            return fil_fileobj
        fil_fileobj = create_filterbank_fileobj('INTEN.fil',
                                                *INTEN.shape,
                                                983.04*10**(-6),
                                                1, 1)
        fil_fileobj.cwrite(FinalData)
        fil_fileobj.close()
        
    def sigpyproc_waterfaller(self,
                             filterbank,
                             start,
                             end,
                             dm):
        ...
        fil_file=FilReader(filterbank)
        print(fil_file.header)
        
        fil_data=fil_file.readBlock(0, fil_file.header['nsamples'])
        fil_data=fil_data.dedisperse(self.dm)
        t_down=1
        f_subband=256
        fil_data= fil_data.downsample(t_down,f_subband)
        ...
        fbins=np.round(np.linspace(0,fil_data.shape[0],10))
        ax2 = fig.add_subplot(spec[1])
        imwf = ax2.imshow(np.flip((fil_data[:,self.start:self.end]),axis=0),
                          aspect="auto",
                          vmax=10, vmin=-10)
        ax2.set_yticks(fbins)
        ax2.set_yticklabels(np.round(np.flip(np.linspace(400,800,10))))
        ax2.set_ylabel("Frequency (MHz)", fontsize=18)
        tbins=np.round(np.linspace(0,(fil_data[:,self.start:self.end]).shape[1],10),1)
        ax2.set_xticks(tbins)
        ax2.set_xticklabels(np.round(np.linspace(self.start/1000,self.end/1000,10),2))
        ax2.set_xlabel("Time (s)", fontsize=18)
       ...
       # Adjust the colorbar on this axes alongside tables
        ...
        fig.subplots_adjust(.1, .1, 
                            .95, .95,
                              0, 0)
        fig.savefig('Sigpyproc_Waterfall_Timeseries.jpg', dpi=300)
